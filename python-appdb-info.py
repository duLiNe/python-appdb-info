...
__date__      = "$Date: 06/10/2020 11:15:27"
...
def appdb_call_v2(path):
	''' 
	   Connect to the EGI AppDB server and get the list of 
	   providers supporting the given VO 
	'''
	
	conn = http.client.HTTPSConnection("appdb-pi.egi.eu")
...
def get_resource_tpl(providerID):
	...
	try:
		...
		for resource_tpl in data['appdb:appdb']['virtualization:provider']['provider:template']:
			vo_name = resource_tpl['vo:vo']['#text']
...
def get_os_tpl(providerID):
	...
	try:
		# E.g.: https://appdb.egi.eu/rest/1.0/va_providers/8253G
		data = appdb_call_v2('/rest/1.0/va_providers/%s' %providerID)
		...
		for os_tpl in data['appdb:appdb']['virtualization:provider']['provider:image']:
			try:
				if vo in os_tpl['@voname']:
					#print("- Name = %s [v%s] [%s] " \
					#	%(os_tpl['@appname'], os_tpl['@vmiversion'], providerID))
					...
					if (provider_service_type['@service_type'] == "org.openstack.nova"):
						#print(provider_service_type['@service_type'])
						...
						_url = o.scheme + "://" + o.hostname
							...
							# This case is not true anymore (?)
						#	_os_tpl=os_tpl['@va_provider_image_id'].replace("os_tpl","os")
						#   print("VA_image_ID = http://schemas.openstack.org/template/%s" %_os_tpl)
						#	va_provider_image_id = "%s/%s" %(_url, _os_tpl)
						#else:
						#print("VA_image_ID = %s" %os_tpl['@va_provider_image_id'])
						va_provider_image_id = "%s/%s" %(_url, os_tpl['@va_provider_image_id'])
					else:
						# OCCI cloud providers
						if os_tpl['@va_provider_image_id'].startswith("os_tpl"):
							_os_tpl=os_tpl['@va_provider_image_id'].replace("os_tpl","os")
...
def main():
	...
	try:
		...
		# E.g. https://appdb-pi.egi.eu/rest/1.0/sites?flt=%3Dvo.name:training.egi.eu
		data = appdb_call_v2('/rest/1.0/sites?&flt=%%3Dvo.name%%3A%s' %vo)
